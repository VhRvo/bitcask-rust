...
use crate::{
    ...
    error::Error,
    options::Options,
    utilities::rand_kv::{get_test_key, get_test_value},
};

#[test]
fn test_engine_put() {
    let mut opts = Options::default();
    opts.dir_path = PathBuf::from("/tmp/bitcask-rust-put");
    ...
    assert_eq!(Error::KeyIsEmpty, res6.err().unwrap());
...
fn test_engine_get() {
    let mut opts = Options::default();
    opts.dir_path = PathBuf::from("/tmp/bitcask-rust-get");
    opts.data_file_size = 64 * 1024 * 1024;
    let engine = Engine::open(opts.clone()).expect("failed to open engine");
    ...
    assert_eq!(Error::KeyIsNotFound, res3.err().unwrap());
    ...
    assert_eq!(Error::KeyIsNotFound, res9.err().unwrap());
    ...
    assert_eq!(Error::KeyIsNotFound, res13.err().unwrap());
...
fn test_engine_delete() {
    ...
    assert_eq!(Error::KeyIsEmpty, res5.err().unwrap());
    ...
    assert_eq!(Error::KeyIsNotFound, res10.err().unwrap());
...
//     assert_eq!(res1.err().unwrap(), Error::DatabaseIsUsing);
//
